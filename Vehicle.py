class Vehicle:
    ...
    def add_timestep(self, time, x, y, angle, speed, pos, lane, slope):
        ...
        self.timesteps[timestep.time()] = (
            timestep  # Adiciona um timestep indexado pelo tempo
        )

    def get_timestep(self, time):
        if time in self.timesteps.keys():
            return self.timesteps.get(
                time
            )  # Retorna o timestep para um determinado momento, se existir
        else:
            return None
