...
def generate_drone_coordinates(
    vehicle_coordinates, offset_distance, max_speed, smoothing_factor=0.4
):
    """
    ...
    for i, vehicle_position in enumerate(vehicle_coordinates):
        ...
        if angle is None and i > 0:
            ...
            lat = vehicle_position[0] - (offset_distance / earth_radius) * (
                180 / math.pi
            ) * math.cos(angle)
            lon = vehicle_position[1] - (offset_distance / earth_radius) * (
                180 / math.pi
            ) / math.cos(math.radians(vehicle_position[0])) * math.sin(angle)
            ...
            limited_drone_position = limit_speed(
                (current_lat, current_lon), next_drone_position, max_speed
            )
            ...
            distance = haversine_distance(
                current_lat,
                current_lon,
                limited_drone_position[0],
                limited_drone_position[1],
            )
            speed = distance / (current_time - previous_time)
            speed = round(speed, 2)

            drone_coordinates.append(
                (limited_drone_position[0], limited_drone_position[1], speed)
            )
            previous_time = current_time
...
def generate_drone_coordinates_circular(
    center, radius_meters, num_samples, max_speed=10
):
    """
    ...
    angular_speed = (
        max_speed / radius_meters
    )  # Velocidade angular em radianos por segundo

    for i in range(num_samples):
        angle_radians = angular_speed * i
        lat = lat_center + (radius_meters / earth_radius) * (180 / math.pi) * math.cos(
            angle_radians
        )
        lon = lon_center + (radius_meters / earth_radius) * (180 / math.pi) * math.sin(
            angle_radians
...
def generate_angular_pattern(
    start_point, max_length, max_turns, angle_alpha, num_samples, max_speed
):
    """
    ...
    for i in range(num_samples):
        ...
        if total_distance_covered >= max_length:
            ...
            vertical_displacement = (
                max_length * math.sin(turn_angle) * current_direction
            )
            horizontal_displacement = (
                max_length * math.cos(turn_angle) * current_direction
            )

            # Atualiza latitude e longitude
            lat += (vertical_displacement / earth_radius) * (180 / math.pi)
            lon += (
                horizontal_displacement / (earth_radius * math.cos(math.radians(lat)))
            ) * (180 / math.pi)
            ...
            lat += (
                (distance_per_sample * current_direction)
                / earth_radius
                * (180 / math.pi)
            )
...
def generate_tractor_pattern(
    ...
    distance_degrees = (
        (distance_per_sample * current_direction_main) / earth_radius * (180 / math.pi)
    )

    for i in range(num_samples):
        if i > 0:
            prev_lat, prev_lon, _ = coordinates[-1]
            speed = haversine_distance(
                prev_lat, prev_lon, lat, lon
            )  # Distância percorrida em 1 segundo, que é a velocidade
        else:
            speed = 0  # Inicialmente a velocidade é zero
...
def generate_square_pattern(
    center_point, side_length, angle_degrees, num_samples, max_speed
):
    """
    ...
    def rotate_point(lat, lon, center_lat, center_lon, cos_angle, sin_angle):
        ...
        rotated_lon = (
            rotated_lon_m
            / (earth_radius * math.cos(math.radians(center_lat)) * (math.pi / 180))
            + center_lon
        )
...
def generate_angular_pattern(
    ...
    for i in range(num_samples):
        ...
        if distance_covered >= max_length:
            ...
            # Atualiza latitude e longitude
            lat += (
                (vertical_displacement / earth_radius)
                * (180 / math.pi)
                * current_direction
            )
            lon += (
...
def create_drone_angular_pattern(
    timesteps, drone_id, start_point, max_length, max_turns, angle_alpha, max_speed
):
    drone_coordinates = generate_angular_pattern(
        start_point, max_length, max_turns, angle_alpha, timesteps, max_speed
    )
    ...
    for time in range(timesteps):
        ...
        if (x_current, y_current) != (0, 0):
            drone.add_timestep(
                time, x_current, y_current, "0", round(speed, 2), "0", "0", "0"
            )
...
def create_drone_following_object(
    timesteps, drone_id, vehicle, offset_distance, max_speed
):
    ...
    drone_coordinates = generate_drone_coordinates(
        coordinates, offset_distance, max_speed
    )
    drone = Vehicle(drone_id, "VANT")
...
def create_drone_circular_point(timesteps, drone_id, center, radius_meters, max_speed):
    drone_coordinates = generate_drone_coordinates_circular(
        center, radius_meters, timesteps, max_speed
    )
    drone = Vehicle(drone_id, "VANT")
...
def create_drone_tractor_pattern(
    timesteps,
    drone_id,
    start_point,
    width_between_tracks,
    ...
    drone_coordinates = generate_tractor_pattern(
        start_point,
        width_between_tracks,
        max_length,
        max_turns,
...
def create_drone_square_pattern(
    timesteps, drone_id, center_point, side_length, angle_degrees, max_speed
):
    drone_coordinates = generate_square_pattern(
        center_point, side_length, angle_degrees, timesteps, max_speed
    )
    drone = Vehicle(drone_id, "VANT")
...
def generate_generic_pattern(
    start_point, distance_lists, angles_list, num_samples, max_speed
):
    """
    ...
    distance_to_cover = meters_to_geo(distance_lists[turn]) / math.sqrt(
        2
    )  # y**2=math.sqrt(2x**2)
...
def create_drone_generic_pattern(
    timesteps, drone_id, start_point, distance_lists, angles_list, max_speed
):
    drone_coordinates = generate_generic_pattern(
        start_point, distance_lists, angles_list, timesteps, max_speed
    )
    drone = Vehicle(drone_id, "VANT")
